...
`sudo apt install ./AIdrone_0.1.12_amd64.deb`, then unplug/replug the board. The
...
cd app
bun install
bun run tauri build --bundles nsis           # Windows -> AIdrone_0.1.12_x64-setup.exe
bash src-tauri/installer/linux/build-deb.sh  # Ubuntu  -> AIdrone_0.1.12_amd64.deb
```
...
bash src-tauri/installer/linux/verify-deb.sh \
  src-tauri/target/release/bundle/deb/AIdrone_0.1.12_amd64.deb
```
